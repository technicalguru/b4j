...
public class ManagementTrackingReport extends AbstractFileReport {
	...
	private static Log log = LogFactory.getLog(ManagementTrackingReport.class);
	...
	public void init(Configuration config) throws ConfigurationException {
		...
		while (groups.hasNext()) {
			...
			Configuration groupConfig = ((SubnodeConfiguration)config).configurationAt("severityGroup("+grno+")");
		...
		for (int i=0; i<DefaultBugzillaBug.SEVERITIES.length; i++) {
			if (!severityGroups.containsKey(DefaultBugzillaBug.SEVERITIES[i])) allSeverities.add(DefaultBugzillaBug.SEVERITIES[i]);
		}
	}
