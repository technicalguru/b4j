...
/**
 * Creates a Change Log from all closed bugs (see {@link BugzillaBug#isClosed()}).
...
public class ChangeLogReport extends AbstractFileReport {
	...
	private static Log log = LogFactory.getLog(ChangeLogReport.class);
...
&lt;/Version&gt;
	...
	public void init(Configuration config) throws ConfigurationException {
		...
		try {
			...
			if (className == null) {
				...
				SubnodeConfiguration rConfig = ((SubnodeConfiguration)config).configurationAt("ReleaseProvider(0)");
		...
		List<String> classNames = config.getList("ChangeLogEntryProvider[@class]");
	 ...
	 * @see b4j.report.BugzillaReportGenerator#registerBug(b4j.core.BugzillaBug)
	 */
	@Override
	public void registerBug(BugzillaBug bug) {
	 ...
	 */
	protected String getBugText(BugzillaBug bug) {
	 ...
	 */
	protected Release getRelease(BugzillaBug bug) {
