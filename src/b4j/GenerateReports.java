...
public class GenerateReports implements Runnable {
	...
	private static Log log = LogFactory.getLog(GenerateReports.class);
	...
	public void run() {
		...
		BugzillaSession session = getMetaInformation().getBugzillaSession();

		// Login
		if (session.open()) {

			// Make the search
			Iterator<BugzillaBug> bugs = session.searchBugs(getMetaInformation().getBugzillaSearchData(), null);
			if (bugs != null) {
				...
				// Iterate on all bugs
				while (bugs.hasNext()) {
					BugzillaBug bug = bugs.next();
					//log.info("new bug: "+bug.getId());
